...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    private boolean shouldAuthentication(String token) {
        return token != null
    ...
    private String resolveToken(HttpServletRequest request) {
        ...
        if (!isBearerToken(header)) return null;
    ...
    private boolean isBearerToken(String header) {
        return StringUtils.hasText(header)
                && header.startsWith(SecurityConstants.PREFIX_BEARER);
    }

    private String extractToken(String header) {
        return header.substring(SecurityConstants.PREFIX_BEARER.length());
    }
}
